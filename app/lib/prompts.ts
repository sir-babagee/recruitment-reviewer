...
- Calculating how long someone has been in a role marked "Present" or "Current"
...
- When calculating tenure, always use ${today} as the end date for roles marked "Present" or "Current".
...
export function buildUserPrompt(job: JobSetup, resumeText: string): string {
  ...
  return `Evaluate the following candidate's resume against this job opening.

REMINDER: Today's date is ${today} (year ${new Date().getFullYear()}). Use this for all tenure and gap calculations.
...
=== INSTRUCTIONS ===
Produce a complete structured screening report for this candidate. Be thorough, fair, and evidence-based. Follow the scoring system exactly. Remember: the current date is ${today} — use it for computing all durations.`;
}
